...
import com.laputa.integration.model.http.ResponseUserEntity;
...
public class HttpsAdminServerTest extends BaseTest {
    ...
    public void adminLoginFlowSupport()  throws Exception {
        ...
        try (CloseableHttpResponse response = httpclient.execute(loadAdminPage)) {
            ...
            //todo add full page match?
            assertTrue(adminPage.contains("Laputa Administration"));
    ...
    public void testGetUserFromAdminPageNoAccess() throws Exception {
        String testUser = "dmitriy@laputa.cc";
    ...
    public void testChangeUsernameChangesPassToo() throws Exception {
        ...
        User user;
        HttpGet getUserRequest = new HttpGet(httpsAdminServerUrl + "/users/admin@laputa.cc-Laputa");
        ...
        user.email = "123@laputa.cc";

        //we are no allowed to change username without cahnged password
        HttpPut changeUserNameRequestWrong = new HttpPut(httpsAdminServerUrl + "/users/admin@laputa.cc-Laputa");
        ...
        user.pass = "123";
        HttpPut changeUserNameRequestCorrect = new HttpPut(httpsAdminServerUrl + "/users/admin@laputa.cc-Laputa");
        ...
        HttpGet getNonExistingUserRequest = new HttpGet(httpsAdminServerUrl + "/users/admin@laputa.cc-Laputa");
        try (CloseableHttpResponse response = httpclient.execute(getNonExistingUserRequest)) {
            assertEquals(404, response.getStatusLine().getStatusCode());
        }

        HttpGet getUserRequest2 = new HttpGet(httpsAdminServerUrl + "/users/123@laputa.cc-Laputa");
        try (CloseableHttpResponse response = httpclient.execute(getUserRequest2)) {
            assertEquals(200, response.getStatusLine().getStatusCode());
            String userProfile = consumeText(response);
            assertNotNull(userProfile);
            user = JsonParser.parseUserFromString(userProfile);
            assertEquals("123@laputa.cc", user.email);
    ...
    public void testForceAssignNewToken() throws Exception {
        login(admin.email, admin.pass);
        HttpGet request = new HttpGet(httpsAdminServerUrl + "/users/token/force?email=dmitriy@laputa.cc&app=Laputa&dashId=79780619&deviceId=0&new=123");
